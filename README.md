# RL_mini_project
RL수업의 미니 프로젝트입니다.

requirements.txt는 RL파일속에 1개, 겉에 1개 이렇게 되어 있습니다. 둘중에 아무거나 쓰셔도 괜찮습니다만, 처음 만들어 봐서 버전 이슈가 있을수 있단 점 참고 해주세요.

혹시 몰라서 확인용requirements.txt도 같이 첨부 합니다.

pip install -r requirements.txt


PDF로 보고서를 보실경우 점부된 동명의 txt파일을 참고해주세요.

==================================

조 : 12조

팀원 : 조재혁

120230248 조재혁입니다.

====================================

문제번호

0. oh_jama
   
1. Ancient_Kings

에이전트 타입 -> 선택가능 

dqn , double_dqn , rainforce_base , a2c , 선택하기 귀찮을땐 all

에피소드수 : 500단위로 500보다 많게만 해주세요. 학습을 500단위로 끊어 뒀거든요.


# train mode


python train_rl.py --puzzle 문제번호 --algorithm 에이전트 타입 --episodes 에피소드수(전 10000으로 했습니다.)
======================================================================


# elvaluation mode


# Oh Jama 플레이 시각화 reinforce_baseline
(디폴트가 reinforce_baseline으로 되어 있습니다. 그래서 다른 알고리즘의 경우에는 뒤에 --algorithm 알고리즘 명을 써야합니다.)

python play_oh_jama.py --model results/best_model_Oh_Jama_20251206_162754.pth


# Ancient Kings 플레이 시각화 a2c

python play_ancient_kings.py --model results/best_model_Ancient_Kings_20251206_034924.pth --algorithm a2c


만일 실행이 잘 안 될경우, 백업파일에 있는 걸 쓰세요
